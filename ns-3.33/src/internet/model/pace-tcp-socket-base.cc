 ...
 * @LastEditTime: 2022-06-25 22:41:13
...
PaceTcpSocketBase::AdvertisedWindowSize (bool scale) const
{
    ...
    // std::cout << "scale: " << scale << ", shift: " << (uint64_t)m_rcvWindShift << std::endl; 
    uint32_t w;
...
PaceTcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
    ...
    if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
    {
        ...
        if (ts->GetEcho () != m_lastEcho)
        {
            ...
            m_recentRTT = TcpOptionTS::ElapsedTimeFromTsValue (ts->GetEcho ());     // 最新计算得到的 RTT
            m_ReceRttEstimator->Measurement (m_recentRTT);                          // 平滑计算
            m_lastReceRtt = m_ReceRttEstimator->GetEstimate();                      // 接收端测得的 RTT 的平滑值

            m_ratesample.m_rtt = m_recentRTT;
            m_socketState->m_lastRtt = m_lastReceRtt;                               // 保存到 socket state 中
            // CalculateRttThreshold (m_lastReceRtt);
        }
...
PaceTcpSocketBase::Initialization (void)
{
    ...
    if (m_socketState == nullptr)
    {
        m_socketState = CreateObject<TcpSocketState> ();
    }
...
void PaceTcpSocketBase::AppendAck (Ptr<Packet> packet, TcpHeader& header)
{
    ...
    item->m_rateInfo.m_delivered = m_totalRxBytes;

    m_ackList.insert (m_ackList.end (), pack);

    if (m_copa == nullptr && m_socketState->m_lastRtt != Time ("0s"))
    {        
        m_socketState->m_pacing = true;
        m_socketState->m_cWnd = 1500 * 10;
        m_socketState->m_segmentSize = m_peerSocket->m_tcb->m_segmentSize;
        

        m_copa = CreateObject<TcpCopa> ();
        m_copa->Init(m_socketState);
    }
}
...
PaceTcpSocketBase::SendAck ()
{
    ...
    // if (m_pacingState == PC_STABL)
    // {
    //     if (m_socketState->m_cWnd <= m_peerSocket->BytesInFlight() - m_totalRxBytes + item->m_item->m_rateInfo.m_delivered)
    //     {
    //         return;
    //     }
    // }

    if (m_copa && m_socketState->m_cWnd <= m_peerSocket->BytesInFlight() - m_totalRxBytes + item->m_item->m_rateInfo.m_delivered + m_socketState->m_segmentSize)
    {
        m_ackSentEvent.Cancel ();
        return;
    }
    ...
    for (auto i = header.m_options.begin (); i != header.m_options.end (); ++i)
    {
        if ((*i)->GetKind () == TcpOption::TS)
        {
            ...
            // if (m_pacingState == PC_STABL)
            // {
            //     auto oldsize = header.GetWindowSize ();
            //     auto newsize = m_socketState->m_cWnd.Get() >> m_rcvWindShift;
            //     uint16_t wind = std::min((uint16_t)(newsize), oldsize);
            //     header.SetWindowSize (wind);
            // }
    ...
    PaceAndScheduleNextAck (p, header, item->m_item->m_startSeq);
}


// 真正发送 ACK 时判断当前环境
void
PaceTcpSocketBase::PaceAndScheduleNextAck (Ptr<Packet> packet, TcpHeader header, SequenceNumber32 seq)
{
    uint32_t npacket;
    ...
    if (m_peerSocket != nullptr)
    {
        ...
        case PC_BEGIN:
            ...
            //     m_pacingAckRate = m_socketState->m_pacingRate;
            ...
            //     std::cout << "peer cwnd = " << m_peerSocket->m_tcb->m_cWnd << ", copa cwnd = " << m_socketState->m_cWnd << std::endl;
            //     std::cout << "curr rate = " << m_bandwidth.GetBitRate() / 10000000.0 << "Mbps" << std::endl;
            //     std::cout << "copa rate = " << m_socketState->m_pacingRate.Get ().GetBitRate() / 10000000.0 << "Mbps" << std::endl;
        ...
        case PC_BGREC:
            ...
            // {
            //     m_pacingState = PacingState::PC_STABL;
            //     m_pacingAckRate = m_maxBandwidth;

            //     m_socketState->m_segmentSize = m_peerSocket->m_tcb->m_segmentSize;
            //     m_copa = CreateObject<TcpCopa> ();
            //     // m_socketStatse->m_cWnd = m_bandwidth.GetBitRate () / 2.0 / 8000 * m_socketState->m_lastRtt.Get ().GetMilliSeconds();
                
            //     std::cout << "cwnd = " << m_socketState->m_cWnd << ", rate = " << m_bandwidth.GetBitRate () / 1000000.0 << std::endl;
            //     m_socketState->m_pacing = true;
            //     m_copa->Init(m_socketState);
            //     m_copa->m_isSlowStart = false;
    ...
    if (m_peerSocket != nullptr && m_isAckPacingEnable == true)
    {
        ...
        {
            intervaltime = m_socketState->m_pacingRate.Get ().CalculateBytesTxTime (npacket * GetSegSize ());
        }
...
PaceTcpSocketBase::CalculateBandwidth (Ptr<Packet> packet)
{
    ...
    m_ratesample.m_ackedSacked = packet->GetSize();
...
PaceTcpSocketBase::UpdateAckPacingRate ()
{
    ...
    case PC_BEGIN:
        ...
        if (m_copa!= nullptr)
        {
            m_copa->CongControl (m_socketState, rc, m_ratesample);
            m_pacingAckRate = m_socketState->m_pacingRate;

            // std::cout << "cwnd = " << m_socketState->m_cWnd << ", rate = " << m_socketState->m_pacingRate << std::endl;
        }
        break;
